...
export default {
  ...
  methods: {
    renderAncestor (createElement, elements) {
      return createElement('div', { attrs: { class: 'tile is-ancestor' } }, elements.map((element, i) => {
        return createElement('div', { attrs: { class: 'tile is-parent' + (this.cols && this.cols[i] ? ` ${this.cols[i]}` : '') } }, [element])
      }))
    }
  }
}
